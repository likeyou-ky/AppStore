...
def settings_success(request):
    if request.method == "POST":
        ...
        valid = c.fetchall()
        if not valid:
            c.execute("INSERT INTO buddies VALUES ('{0}','{1}',{2},{3},'{4}','{5}','{6}','{7}','{8}');".format(email, education, height, rate_per_hour, interest1, interest2, interest3, interest4, interest5))
        else:
            ...
            paramlist = []
            for i in [update_height, update_education, update_rate, interest1, interest2, interest3, interest4, interest5]:
                if len(i)!=0:
                    paramlist.append(i)
